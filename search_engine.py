...
def search(index, query):
    ...
    listA = index.get(query[0], default)

    # if the key is not availble, it returns none not an empty list
    # need conditional to make sure it returns an empty list
    # what happens when a key is not ina dictionary, let it return an empty list
    
    query.pop(0)

    new_query = query

    for queries in new_query:
        listB = index.get(queries, default)
        listA = common(listA, listB)
